...
- `SettingsView` is a SwiftUI grouped `Form` shown in an `NSWindow` at `.floating` level. It is 480pt wide and uses `.fixedSize(horizontal: false, vertical: true)` so the form reports its full content height instead of scrolling; `AppState.showSettings()` sets the window's content size to the hosting view's `fittingSize` and enables `NSHostingController.sizingOptions = [.preferredContentSize]` so the window follows later changes (a two-line hook status, for example). Rows: head size, snap distance, status indicator, "Show children for subagents", the hook status row with the "Reinstall hooks" button, terminal font/size (posting `.terminalFontChanged` / `.headSizeChanged` / `.subagentChildrenVisibilityChanged` notifications that `AppState` applies live), and the Claude Code flags.
