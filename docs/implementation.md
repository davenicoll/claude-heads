...
- `SettingsView` is a SwiftUI `Form` shown in an `NSWindow` at `.floating` level: head size, snap distance, status indicator, "Show children for subagents", terminal font/size (posting `.terminalFontChanged` / `.headSizeChanged` / `.subagentChildrenVisibilityChanged` notifications that `AppState` applies live), and the Claude Code flags.
