...
- Children persist across turns while their agent runs: a child leaves on its own `SubagentStop`, or when the parent's `Stop` payload lists it in `background_tasks` with a finished status (`completed`, `failed`, `killed`, `cancelled`). Being absent from the list is not evidence (a teammate is listed under a different id than its `SubagentStart`), so unlisted children stay, and a `Stop` without the list removes nothing; the process exiting clears them all
