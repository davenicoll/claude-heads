...
└── ClaudeHeads/                       # ClaudeHeadsCore library
    ...
    │   └── HookInstaller.swift        # HookSettingsMerge (byte-preserving install/remove of our hooks in settings.json) + HookInstaller (file I/O, backup, status)
    ├── Utilities/
...
Tests/ClaudeHeadsCoreTests/            # XCTest: SnapEngine, PathColorGenerator, HeadGeometry, Codable models, hook marker/notify.sh, HookSettingsMerge fixtures
```
...
`HookWatcher` is a file-system watcher (`DispatchSource.makeFileSystemObjectSource`) on `~/.claude-heads/hooks` that looks for `<uuid>.done` marker files, deletes them and calls `onTaskComplete(uuid, backgroundTasks)`. It also parses `<uuid>.<agent_id>.start` and `<uuid>.<agent_id>.stop` markers from the `SubagentStart`/`SubagentStop` hooks into `onSubagentStart(uuid, agentID, agentType, description)`/`onSubagentStop`, which `AppState` uses to maintain each head's orbiting `children`. Every marker contains the hook event JSON verbatim, parsed by `HookPayload` with `JSONSerialization` (tolerant of empty or malformed content); a payload's `background_tasks` array is delivered via `onBackgroundTasks` and merged into the children by the pure `SubagentInstance.merging`/`reconciling` functions (relabel by id, add alive subagents never seen starting; a Stop removes only children the list reports with a finished status, and removes nothing without a list). Each child's caption is `SubagentInstance.label` (description, else type, else id prefix). It is the single source of truth for `notify.sh`, which it rewrites on every launch; the script reads `CLAUDE_INSTANCE_ID` (exported into each spawned `claude`) and writes stdin into the marker atomically (temp file + `mv`). `AppState` owns the watcher and routes `onTaskComplete` to `handleHookTaskComplete`, which is authoritative over the PTY-activity heuristic (see Process Lifecycle). Without the hook configured, the wave still fires from PTY idle detection.

`HookInstaller` keeps those hooks configured in the global `~/.claude/settings.json` (never a project-level file) while the app runs. `AppState` creates it after `HookWatcher` (so `notify.sh` exists) and unconditionally calls `install()` at launch and `uninstall()` from `shutdown()` on every quit path; there is no setting, and "Show children for subagents" only affects rendering (children are tracked while hidden, so all three hooks stay installed). "Reinstall hooks" in Settings calls `reinstall()` (strip stale entries, then install, in one write). The text edit lives in the pure `HookSettingsMerge` enum: a small strict-JSON scanner records the byte range of every value, `JSONSerialization` is used only to decode string tokens and to validate, and the change is a single splice at the start of the `"hooks"` object (or the root object) formatted with the file's own indentation unit and line ending. Removal is the inverse splice (`removalRange` mirrors `insert`), deleting only command entries whose path ends in `.claude-heads/hooks/notify.sh` plus any matcher group, event array or `"hooks"` object we emptied; everything outside those entries is preserved byte-for-byte, so install followed by uninstall normally returns the original bytes. Duplicate keys in the root or `hooks` object are refused (`JSONSerialization` keeps the first, `JSON.parse` the last). Every result is re-parsed and compared to the original with our entries stripped before it is returned; on any doubt nothing is written and `HookInstaller.status` becomes `.failed(reason)`. `HookInstaller` itself targets `$CLAUDE_CONFIG_DIR/settings.json` or `~/.claude/settings.json`, resolves symlinks (`~/.claude` is often a symlink into a dotfiles repo; a dangling link is reported rather than replaced), writes a one-time `~/.claude-heads/settings.json.claude-heads.bak`, and writes via a 0600 temp file + `rename` in the same directory, preserving the original mode. Hooks only affect `claude` processes started after the write, which is always true for the app's own heads. A crash leaves the entries in place harmlessly (`notify.sh` exits 0 without `CLAUDE_INSTANCE_ID`) and the next launch is idempotent. See the Hook Setup section of the README for the user-facing behaviour and manual fallback.
